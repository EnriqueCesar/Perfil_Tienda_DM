...
function sec(v){if(!valid(v))return '—';let s=Math.round(Math.abs(Number(v)));let h=Math.floor(s/3600),m=Math.floor((s%3600)/60),r=s%60;return h>0?`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`:`${String(m).padStart(2,'0')}:${String(r).padStart(2,'0')}`;}
...
function rowsByDM(source,dms){const set=new Set(dms);return (source||[]).filter(r=>set.has(r.dm));}
...
function chartCard(cfg){
  ...
  const k=cfg.ytd===false?latest(vals):ytd(vals,cfg.agg||'avg');
...
function iconCards(id,obj,type){const entries=Object.entries(obj).filter(([k,v])=>valid(v)&&v>0).sort((a,b)=>b[1]-a[1]).slice(0,4);$(id).innerHTML=entries.map(([k,v])=>`<div class="icon-card"><span class="icon-badge">${iconFor(k,type)}</span><div><b>${cleanLabel(k)}</b><small>${fmt(v,'pct')}</small></div></div>`).join('')||'<div class="note">Sin data disponible</div>';}
function dough(id,obj){const labels=Object.keys(obj).filter(k=>valid(obj[k])&&obj[k]>0),vals=labels.map(k=>obj[k]);makeChart(id,{type:'doughnut',data:{labels,datasets:[{data:vals,borderWidth:2,backgroundColor:['#006241','#00754a','#00a862','#d4e9e2','#8b6f47','#cba258']}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{boxWidth:12}},tooltip:{callbacks:{label:c=>`${c.label}: ${fmt(c.raw,'pct')}`}}}}});}
